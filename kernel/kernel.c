...
// --- Загрузчик внешних приложений (Каркас) ---
void execute_external_app(char* filename) {
    kprint("Preparing to load: ");
    kprint(filename);
    kprint("\n");
    kprint("FS file reading and memory allocation pending implementation.\n");

    // В будущем здесь будет:
    // 1. Поиск файла через fs.c
    // 2. Выделение памяти (например, uint8_t* load_address = (uint8_t*)0x2000000)
    // 3. Чтение файла с диска в эту память
    // 4. void (*app_entry)() = (void (*)())load_address;
    // 5. app_entry();
}
...
void execute_command(char *input) {
    ...
    if (compare_string(input, "EXIT") == 0) {
        ...
        kprint("System: HELP, CLEAR, EXIT, UX, INFO, TIME, LS, FORMAT, TOUCH, ECHO, SUDO, RUN\n");
    }
